...
Once you got the executable you can use it in two ways: you can pass the API Token, the Zone ID and the record name as command line arguments or you can use a TOML configuration file, located in `/etc/cloudflare-ddns/config.toml` on Linux systems; [here's the template](config.toml). If you prefer, you can even use a configuration file in a custom location, using `--config file-path`.
